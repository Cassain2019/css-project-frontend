...
export default function AccountGroup() {
    ...
    //level1
    useEffect(() => {
        let options = classes.filter((arr) => arr.accountType === data.accountType)
    ...
    const onSubmit = (e) => {
        ...
        if (!Object.values(data).every((element) => element !== "") === false) {
            axios.post("http://localhost:4000/groups/", data)
                .then((res) => {
                    toast.dismiss();
                    if (res.data === "Added") {
    ...
    return <div id="accountGroup" className="modal fade" role="dialog">
        <div className="modal-dialog modal-dialog-centered modal-lg ">
            <div className="modal-content">
                <div className="modal-header">
                    <button type="button" className="close" data-dismiss="modal">&times;</button>
                    <h4 className="modal-title">Account Group</h4>
                </div>
                <div className="modal-body">
                    <form onSubmit={onSubmit}>
                            ...
                            <div className="form-group col-md-6">
                                ...
                                <Select
                                    ...
                                    options={classOptions.map((e, index) => {
                                        return { label: e.name, value: e._id, target: { name: 'accountClass', value: e._id } }
                                    })}
                                    value={classOptions.map((e, index) => {
                                        if (data.accountClass === e._id) { return { label: e.name, value: e._id } }
                                        return null;
                                    })}
                                />
                            </div>
                        ...
                        <div className="col-12 text-center">
                            <button className="customBtn third pulse" type="submit">Create Group</button>
                        </div>
                    </form>
                </div>
            </div>
